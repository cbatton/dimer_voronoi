...
void Dimer::GetParams(string name, int rank_in) {
    ...
    if(input.fail()) {
        ...
        //cout << "k_umb " << k_umb << " bond_umb " << bond_umb << endl;
        getline(input, line);
...
void Dimer::BondForce(vector<float>& force_) {
    ...
    force_scalar -= k_umb*(distance-bond_umb);
...
void Dimer::Equilibriate(int steps) {
    ...
    // Run with k_umb to force it close
    ...
    // Turn off k_umb, now use voronoi cell
    k_umb = 0.0;
    for(int i=0; i<steps; i++) {
        generator = Saru(seed_base, count_step++);
        vector<vector<float>> state_old(state);
...
float Dimer::RDFf1(float r) {
    // Calculate Deserno's f1 function for RDF calculation
    return (1.0-12.0*r*r)*atan(sqrt(4.0*r*r-2));
}

float Dimer::RDFf2(float r) {
    // Calculate Deserno's f2 function for RDF calculation
    float num = 2.0*r*(4.0*r*r-3.0);
    float denom = sqrt(4.0*r*r-2.0)*(4.0*r*r+1.0);
    return 2.0/3.0*r*r*8.0*r*atan(num/denom);
}
...
void Dimer::RDFAnalyzer() {
    ...
    vector<float> edges_l(num_bins_gr,0.0);
    for(int i=0; i<num_bins_gr; i++) {
        edges[i] = (i+0.5)*dr;
        edges_l[i] = i*dr;
        if(edges_l[i] < 0.5*box[0]) {
            if(edges_l[i+1] < 0.5*box[0]) {
                volume_change[i] = 4.0*M_PI/3.0*(pow(i+1,3)-pow(i,3))*pow(dr,3);
            }
            else {
                float volume_0 = 4.0*M_PI/3.0*pow(i*dr,3);
                float volume_1 = -M_PI/12*(3-36*pow((i+1)*dr/box[0],2)+32*pow((i+1)*dr/box[0],3));
                volume_change[i] = volume_1*pow(box[0],3)-volume_0;
            }
        }
        else if(edges_l[i] < 0.5*sqrt(2)*box[0]) {
            if(edges_l[i+1] < 0.5*sqrt(3)*box[0]) {
                float volume_0 = -M_PI/12*(3-36*pow(i*dr/box[0],2)+32*pow(i*dr/box[0],3));
                float volume_1 = -M_PI/12*(3-36*pow((i+1)*dr/box[0],2)+32*pow((i+1)*dr/box[0],3));
                volume_change[i] = (volume_1-volume_0)*pow(box[0],3);
            }
            else {
                float volume_0 = -M_PI/12*(3-36*pow(i*dr/box[0],2)+32*pow(i*dr/box[0],3));
                float volume_1 = -M_PI/4+3*M_PI*pow((i+1)*dr/box[0],2)+sqrt(4*pow((i+1)*dr/box[0],2)-2);
                volume_1 += RDFf1((i+1)*dr/box[0]);
                volume_1 += RDFf2((i+1)*dr/box[0]);
                volume_change[i] = (volume_1-volume_0)*pow(box[0],3);
            }
        }
        else if(edges_l[i] < 0.5*sqrt(3)*box[0]) {
            float volume_0 = -M_PI/4+3*M_PI*pow(i*dr/box[0],2)+sqrt(4*pow(i*dr/box[0],2)-2);
            volume_0 += RDFf1(i*dr/box[0]);
            volume_0 += RDFf2(i*dr/box[0]);
            float volume_1 = -M_PI/4+3*M_PI*pow((i+1)*dr/box[0],2)+sqrt(4*pow((i+1)*dr/box[0],2)-2);
            volume_1 += RDFf1((i+1)*dr/box[0]);
            volume_1 += RDFf2((i+1)*dr/box[0]);
            volume_change[i] = (volume_1-volume_0)*pow(box[0],3);
    ...
    for(int i=0; i<4; i++) {
        ...
        myfile << "dr " << dr << endl;
        for(int j=0; j<num_bins_gr; j++) {
            myfile << std::scientific << edges[j] << " " << std::scientific << g_r_storage[i][j] << "\n";
        }
        myfile.close();
    }
}
